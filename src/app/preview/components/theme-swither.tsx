    ...
    <div className="relative">
      <div className="flex items-center">
        <button
          id="theme-button"
          type="button"
          className="flex h-8 items-center gap-1.5 rounded-md border border-neutral-200 bg-white px-2.5 text-sm text-neutral-700 hover:bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800"
          onClick={() => {
            const dropdown = document.getElementById('theme-dropdown')
            if (dropdown) {
      ...
      >
        ...
        {availableThemes.map((theme) => (
          ...
          >
            ...
            <span className={theme === currentTheme ? 'font-medium' : ''}>{theme}</span>
          </button>
        ))}
      </div>
    </div>

...
export function standardizeOklchFormat(oklch: string): string {
  ...
  if (match && match.length >= 4) {
    ...
    const lightness = parseFloat(match[1] || '0') * 100
    const chroma = parseFloat(match[2] || '0')
    const hue = parseFloat(match[3] || '0')
    return `oklch(${lightness.toFixed(3)}% ${chroma.toFixed(3)} ${hue.toFixed(3)})`
  }
...
export function extractOklchComponents(oklch: string): {
  ...
  return {
    lightness: parseFloat(match[1] || '0'),
    chroma: parseFloat(match[2] || '0'),
...
export function hexToOklch(hex: string): string {
  ...
  const chroma = Math.max(Math.max(rNorm, gNorm), bNorm) - Math.min(Math.min(rNorm, gNorm), bNorm)
  ...
  return `oklch(${Math.round(lightness * 100)}% ${chroma.toFixed(3)} ${hue.toFixed(3)})`
}
...
export function generateBaseColors(primaryColor: string): {
  ...
  const baseContent = `oklch(21% ${(chroma * 0.8).toFixed(3)} ${hue.toFixed(3)})`
...
export function generateDarkBaseColors(primaryColor: string): {
  ...
  const base100 = `oklch(25% ${(chroma * 0.1).toFixed(3)} ${darkHue.toFixed(3)})` // Dark background
  const base200 = `oklch(23% ${(chroma * 0.08).toFixed(3)} ${darkHue.toFixed(3)})` // Slightly darker
  const base300 = `oklch(21% ${(chroma * 0.06).toFixed(3)} ${darkHue.toFixed(3)})` // Even darker

  // Content should be light for good contrast with dark backgrounds
  const baseContent = `oklch(98% ${(chroma * 0.05).toFixed(3)} ${(darkHue + 5).toFixed(3)})`

...
export function ThemeGenerator() {
	...
	const handleSchemeSelect = (schemeName: string) => {
		const selectedSchemeObj = recommendedSchemes.find(
			(s) => s.name === schemeName,
		);
				...
				<TabsContent value="colors" className="space-y-6">
					...
					{selectedColor && recommendedSchemes.length > 0 && (
						<div className="space-y-4">
							<h3 className="font-medium text-lg">推荐的颜色方案</h3>

							<div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
								{recommendedSchemes.map((scheme) => (
									<Card
										key={scheme.name}
										className={`cursor-pointer transition-all hover:border-primary ${selectedScheme === scheme.name ? "border-primary ring-1 ring-primary" : ""}`}
									...
									>
										...
										<CardContent className="p-4 pt-0">
											<div className="flex space-x-2">
												<div
													className="flex h-10 w-10 items-center justify-center rounded-md border font-medium text-lg"
													style={{

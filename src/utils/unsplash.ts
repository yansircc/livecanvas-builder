...
function markAsRecentlyUsed(url: string): void {
	...
	const existingIndex = recentlyUsedImages.findIndex(
		(item) => item.url === url,
	);
	if (existingIndex >= 0) {
...
function getRandomImage(category: ImageCategory): string {
	...
	const unusedImages = UNSPLASH_IMAGES[category].filter(
		(url) => !isRecentlyUsed(url),
	);
	...
	const randomIndex = Math.floor(
		Math.random() * UNSPLASH_IMAGES[category].length,
	);
...
export function getUnsplashImage(category?: ImageCategory): string {
	...
	const selectedCategory: ImageCategory =
		...
		SUPPORTED_CATEGORIES[
			Math.floor(Math.random() * SUPPORTED_CATEGORIES.length)
		] ||
		"business";

...
export function extractAndParseJSON<T>(input: string): T | null {
	...
	if (markdownExtracted) {
		try {
			...
			console.log(
				"Markdown extraction succeeded but parsing failed, continuing with other methods",
			);
		}
	}
	...
	try {
		...
		console.log(
			"Preprocessed input parsing failed, continuing with tokenization",
		);
	}
	...
	if (extracted) {
		try {
			...
			console.log(
				"Balanced JSON extraction failed, continuing with partial extraction",
			);
		}
	}
...
function tokenize(input: string): Token[] {
	...
	if (context.currentToken) {
		addToken(
			context,
			determineTokenType(context.currentToken),
 ...
 */
function processMultiCommentEndState(
	...
	if (codeMatch) {
		...
		if (advicesMatch?.[1]) {
			try {
				...
				if (adviceMatches) {
					minimalObject.advices = adviceMatches.map((a) =>
						a.replace(/^["']|["']$/g, ""),
					);
				}
			}
